...
interface PageTransitionContextProps{
  ...
  setEnterTimeline : React.Dispatch<React.SetStateAction<gsap.core.Timeline>>
}
...
const PageTransitionProvider = ({ children } : {children : React.ReactNode}) => {
  const [exitTimeline, setExitTimeline] = useState(() => gsap.timeline({ paused: true, repeat : 0}));
  const [enterTimeline, setEnterTimeline] = useState(() => gsap.timeline({ paused: true , repeat : 0}));

  return (
    <PageTransitionContext.Provider value = {{exitTimeline,setExitTimeline, enterTimeline, setEnterTimeline}}>
        {children}
