...
const Salutations = () => {
    ...
    useGSAP(() => {
        const salutationsEl = gsap.utils.selector(containerRef)('.salutations');
        ...
        for (let i = 1; i < salutations.length ; i++) {
            if(i < salutations.length - 1){
                salutationsTl.to(salutationsEl, {yPercent : i*100/(salutations.length), duration : .5, ease : "power3.out"}, (i === 1  )? "" : "<+=1.5");
            }else{
                salutationsTl.to(salutationsEl, {yPercent : i*100/(salutations.length), duration : .5, ease : "power3.out", onComplete : () => {salutationYSetter(0)}}, "<+=1.5");
            }
        }
    }, {scope : containerRef})
  return (
